...
@Getter
@ToString
public class TempDocAprvResp<T> {
    ...
    public TempDocAprvResp(TempDocAprvPageReq tempDocAprvPageReq, List<T> pageResponseList, int total){
        if(total <= 0){
            return;
        }

        this.page = tempDocAprvPageReq.getPage();
        this.size = tempDocAprvPageReq.getSize();
        this.direction = tempDocAprvPageReq.getDirection();
